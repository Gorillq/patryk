...
def doLogin(request):
    ...
    if not (email_id and password):
        messages.error(request, "Wprowadź dane logowania")
        return render(request, 'login_page.html')
...
def doRegistration(request):
    ...
    if not (email_id and password and confirm_password):
        messages.error(request, 'Proszę uzupełnić wszystkie pola')
        return render(request, 'registration.html')
     
    if password != confirm_password:
        messages.error(request, 'Hasła muszą być jednakowe')
        return render(request, 'registration.html')
 
    is_user_exists = CustomUser.objects.filter(email=email_id).exists()
 
    if is_user_exists:
        messages.error(request, 'Konto przypisane do tego adresu email znajduje się w bazie danych')
        return render(request, 'registration.html')
 
    user_type = get_user_type_from_email(email_id)
 
    if user_type is None:
        messages.error(request, "zły format adresu email'")
        return render(request, 'registration.html')
